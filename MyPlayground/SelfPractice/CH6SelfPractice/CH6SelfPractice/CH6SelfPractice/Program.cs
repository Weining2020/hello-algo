...
/* 开放寻址哈希表 */ //开放寻址哈希表。底层是环形数组。这不是在用链式地址来改进哈希表，忘掉链式地址改进哈希表用的 List<List<Pair>>结构吧！跟这无关。
class HashMapOpenAddressing
{
    ...
    Pair[] buckets; // 桶数组 //底层用的是Pair数组，即我们追求不同的key（即使有可能对应同一哈希值）的键值对，应存放在Pair数组的不同index上。
    Pair TOMBSTONE = new(-1, "-1"); // 删除标记 //TOMBSTONE字段依然是一个Pair实例，只是其中保存很离谱的值以警示这是个TOMBSTONE而不是有效有意义的的Pair实例，比如这里-1 和 "-1" 是人为设定的特殊值，表示这是一个占位键值对，并没有实际的意义，纯粹是为了标记那些已经被删除的槽位。
    ...
    //有了这个FindBucket()，我们不再用到HashFunc()，而是永远通过FindBucket()来 根据逻辑 找到用户输入key所应该对应的桶索引（return可能是三种情况，在Get()注释中写了）。FindBucket()并不是执行哈希表查询操作，哈希表查询操作是后面的Get()。
    //这个方法非常的复杂，需要多思考，很难。
    int FindBucket(int key)
    { //对整个FindBucket()方法做个总结：如果目标键值对存在，FindBucket() 返回其索引。如果目标键值对不存在，FindBucket() 返回一个适合插入的索引（null 或 TOMBSTONE 的位置）。
        ...
        while (buckets[index] != null)
        {
            ...
            {
                // 若之前遇到了删除标记，则将键值对移动至该索引处。代码if (firstTombstone != -1)用来判断之前是否遇到过删除标记。//目的是，如果在找到对应的桶之前，我们曾碰到过TOMBSTONE，则应该把我们找到的目标键值对放到TOMBSTONE所在的索引位置，因为这样做之后，以后的其他操作效率会更高（即目标值离合理的索引位置更接近了）。
                if (firstTombstone != -1)
                {
            ...
            // 计算桶索引，越过尾部则返回头部
            index = (index + 1) % capacity; //这种越过尾部返回头部继续遍历的情况确实有，继续执行代码就是了，最坏的情况就是本来全部的索引位置上都有有效Pair实例，无法找到一个buckets[index] == null的index来停止while循环，因此程序陷入死循环，其实这种情况肯定也触发了 负载因子过高而执行扩容，因此这个情况不会真实发生。//如果要实现平方探测而不是线性探测，index = (index + 1) % capacity 这一行代码需要改变。在平方探测中，你需要按照平方数列（1, 4, 9, 16...）来增加偏移量，而不是简单地加1。实现平方探测的代码应该类似于：index = (originalIndex + i*i) % capacity;让Claude写代码。
        }
    ...
    /* 删除操作 */ //在开放寻址哈希表中，删除操作的核心挑战是不能简单地将槽位设置为 null，否则可能破坏探测路径，导致查找和插入操作出现问题。因此，删除操作会使用特殊的占位标记（TOMBSTONE）来表示被删除的槽位。
    ...
    void Extend()
    {
        ...
        { //这里没写Pair？是没事的，往下看。
            ...
            { //这里的if是处理有效键值对，排除了TOMBSTONE和null的情况。
                Put(pair.key, pair.val);
            }
        }
    }

    /* 打印哈希表 */ //这个比较直观，三种情况都打印出来。
    public void Print()
    {
